...
- Python 3 backend with dropdown-backed lookups sourced from [`lucky0wl-sketch/Engels`](https://github.com/lucky0wl-sketch/Engels) under `output/`
...
- Extracted lookup JSON under `../Engels/output`, or a custom path passed with `--extraction-output`
...
```bash
git remote add origin git@github.com:YOUR_USERNAME/mamas-toolbox.git
git branch -M main
...
```bash
git remote add origin https://github.com/YOUR_USERNAME/mamas-toolbox.git
git branch -M main
git push -u origin main
```
...
- The editor now reads prebuilt lookup JSON from `../Engels/output`, which in turn is generated by [`lucky0wl-sketch/Engels`](https://github.com/lucky0wl-sketch/Engels) from `master_data` plus English text asset bundles under `assets/revisions`.
